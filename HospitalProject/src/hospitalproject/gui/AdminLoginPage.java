...
import hospitalproject.utils.LoginValidator;
...
public class AdminLoginPage implements ActionListener {
    ...
    public AdminLoginPage() {
        ...
        ImageIcon adminIconImage = new ImageIcon("src/hospitalproject/images/admin_icon.png");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == buttonSubmit) {
            ...
            try {
                ...
                boolean authenticated = LoginValidator.isAuthenticated(username, password);
                boolean anotherAuthenticated = anotherAutheticated(username, password);

                if (authenticated) {
                    Main.changeActiveScreen("adminDashboard");
                    frame.dispose();
                } else {

                    if (anotherAuthenticated) {
                        Main.changeActiveScreen("adminDashboard");
                        LoggedInUser.setLoggedName("Admin");
                        frame.dispose();
                    } else {
    ...
    private boolean anotherAutheticated(String username, String password) throws Exception {
        ...
        for (Admin admin : adminsArr) {

            if (admin.getUsername().equals(username) && PasswordEncryptDecrypt.decrypt(admin.getPassword()).equals(password)) {
                return true;
            }
